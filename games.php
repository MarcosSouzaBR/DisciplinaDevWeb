...
function mostraMensagens(){
		$con = conectaDB();
		$result = mysqli_query($con,"SELECT usuarios.nome as remetente,forum.titulo, forum.mensagem, forum.cod FROM forum,usuarios WHERE usuarios.cod  =  forum.codUsuario"); //eh retornada nesta consulta o FORUM 		
	...
	if(@$_REQUEST['action'] == "recuperaUsuarios") //recupera lista de nomes dos USUARIOS
	{
	...
	if(@$_REQUEST['action'] == "insMensagem") //insere MENSAGEM
	{
		$con = conectaDB();
	...
	if(@$_REQUEST['action'] == "delJogo") //remove Jogo
	{
		...
		if(@$_REQUEST['action'] == "delMensagem") //remove MENSAGEM
	{
		$con = conectaDB();
